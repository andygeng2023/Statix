...
from sqlalchemy import create_engine, String, DateTime, Float, Integer, UniqueConstraint
...
def record_view(t):
    with Session() as s:
        s.query(View).filter_by(user_id=uid(),ticker=t.upper()).delete(); s.add(View(user_id=uid(),ticker=t.upper())); s.commit()
def recent(limit=8):
