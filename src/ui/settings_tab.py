...
provider=st.selectbox(t("provider",new_lang),PROVIDERS,index=PROVIDERS.index(s["provider"]) if s["provider"] in PROVIDERS else 0)
...
if st.button(t("save",new_lang),type="primary"):
 save_settings(new_lang,provider); st.session_state["provider_preference"]=provider; st.session_state["language_preference"]=new_lang; st.rerun()
