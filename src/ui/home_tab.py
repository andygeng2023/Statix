...
for c,ticker in zip(cols,["AAPL","MSFT","NVDA","AMZN"]):
 ...
 if c.button("Open",key="hd"+ticker,use_container_width=True): st.session_state["selected_ticker"]=ticker
...
def live_pulse():
 ...
 for c,ticker in zip(cols,["SPY","QQQ","DIA","IWM"]):
  q=quote(ticker); c.metric(ticker,money(q.get("price")),pct(q.get("change_pct"))); c.caption(f"{q.get('provider','—')} · {q.get('updated_at','—')}")
live_pulse()

st.subheader(t("top_stocks",lang))
cols=st.columns(4)
for c,ticker in zip(cols,["NVDA","AAPL","MSFT","GOOGL"]):
 q=quote(ticker); c.metric(ticker,money(q.get("price")),pct(q.get("change_pct")))
