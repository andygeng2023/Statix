...
header("Statix","Fast market intelligence with a global historical model.")

c1,c2=st.columns([2,1])
with c1:
    if st.button("Search a stock",type="primary",use_container_width=True): st.switch_page("pages/search.py")
with c2:
    if st.button("Open scanner",use_container_width=True): st.switch_page("pages/scanner.py")
...
if recent_tickers:
    st.caption("Recently viewed · "+" · ".join(recent_tickers))
