...
header("Watchlist","Your saved symbols, separated by account when authentication is enabled.")
wl=get_watchlist()
if not wl:
    st.info("Your watchlist is empty. Add stocks from Search.")
for t in wl:
    q=get_quote(t); a,b,c=st.columns([2.4,2,1])
    a.markdown(f"**{t}**")
    b.metric("Price",money(q.get("price")),pct(q.get("change_pct")))
