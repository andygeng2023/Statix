...
async function renderDiscover() { const data = await get('/api/discover'); app.innerHTML = `<div class="page-heading"><p class="eyebrow">MODEL SCANNER</p><h2>Discover</h2><p class="muted">${data.status === 'running' ? 'Scan running in the background.' : 'Ranked market opportunities.'}</p><button id="scan" class="primary">Run scan</button></div>${section('Latest signals', row(data.items))}`; document.querySelector('#scan').onclick = async () => { await fetch('/api/scan', {method: 'POST'}); renderDiscover(); }; bindCards(); }
async function renderStocks(ticker = '') { app.innerHTML = `<div class="page-heading"><p class="eyebrow">STOCK LOOKUP</p><h2>Stocks</h2><form id="search"><input id="symbol" placeholder="Enter ticker" value="${ticker}"><button class="primary">Open</button></form></div><div id="stock-detail"></div>`; document.querySelector('#search').onsubmit = event => { event.preventDefault(); renderStocks(document.querySelector('#symbol').value.toUpperCase()); }; if (ticker) { const item = await get(`/api/stock/${ticker}`); document.querySelector('#stock-detail').innerHTML = section(item.ticker, `<div class="detail">${card(item)}</div>`); bindCards(); } }
